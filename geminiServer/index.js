...
async function waitForFilesActive(files) {
  ...
  for (const file of files) {
    ...
    if (currentFile.state !== "ACTIVE") {
      console.error(
        `File ${currentFile.name} failed to process with state: ${currentFile.state}`
      );
...
const model = genAI.getGenerativeModel({
  ...
  systemInstruction:
    "The user will provide one or more images or one or more PDFs. Analyze the provided data and generate multiple choice questions based on the configuration: number of questions, number of options per question, correct answer index, and difficulty level (Easy, Medium, or Hard).",
});
...
async function generateQuestions(
  ...
  try {
    ...
    if (inputType === "image" || inputType === "pdf") {
      const uploadPromises = inputs.map((inputPath) =>
        uploadToGemini(
          inputPath,
          inputType === "image" ? "image/png" : "application/pdf"
        )
      );
...
async function run() {
  try {
    ...
    const pdfPaths = [path.join(__dirname, "CroatMedJ_56_0169.pdf")];
    const textInput =
      "Today people expect advanced state-of-the-art diagnostic tools... (rest of your text)";

    // const questionsFromText = await generateQuestions(
    //   textInput,
    //   "text",
    //   3,
    //   4,
    ...
    const questionsFromImages = await generateQuestions(
      imagePaths,
      "image",
      4,
      2,
    ...
    if (questionsFromImages)
      console.log(
        "Questions from Images:\n",
        JSON.stringify(questionsFromImages, null, 2)
      );

    // const questionsFromPdfs = await generateQuestions(
    //   pdfPaths,
    //   "pdf",
    //   5,
    //   4,
